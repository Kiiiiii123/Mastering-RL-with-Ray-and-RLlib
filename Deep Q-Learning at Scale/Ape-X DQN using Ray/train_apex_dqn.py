# This is the main script that accepts the training configs and initializes the other components.
import datetime
...
def main(config, max_samples):
    ...
    for i in range(config["eval_num_workers"]):
        eps = 0
        actor = Actor.remote(
            "eval-" + str(i), replay_buffer, parameter_server, config, eps, True
        )
    ...
    while total_samples < max_samples:
        ...
        if (new_total_samples - total_samples) >= config["timesteps_per_iteration"]:
            ...
            tf.summary.scalar(
                "Mean evaluation reward", data=eval_mean_reward, step=total_samples
            )
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "timesteps_per_iteration": 10000,  # decides how frequently we run the evaluation
        "grad_clip": 10,
    }
