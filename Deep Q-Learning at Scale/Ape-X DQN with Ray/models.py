...
def get_Q_network(config):
    ...
    q_estimate_output = Dense(
        config["n_actions"], activation="linear", name="Q_output"
    )(x)
    # Q Model
...
# The trainable model is the model that the learner will optimize.
def get_trainable_model(config):
    ...
    # Trainable model
    loss_output = Lambda(masked_loss, output_shape=(1,), name="Q_masked_out")(
        [sampled_bellman_input, q_estimate_output, mask_input]
    )
    trainable_model = Model(
        inputs=[obs_input, mask_input, sampled_bellman_input], outputs=loss_output
    )
    trainable_model.summary()
    trainable_model.compile(
        optimizer=Adam(lr=config["lr"], clipvalue=config["grad_clip"]),
