# This script includes a learner class that receives samples from the replay buffer, takes gradient steps, and pushes the new Q-network weights to the parameter server.
import time
...
class Learner:
    ...
    def optimize(self):
        samples = ray.get(self.replay_buffer.sample.remote(self.train_batch_size))
        ...
        if samples:
            ...
            actions = np.array([sample[1] for sample in samples]).reshape((N,))
            rewards = np.array([sample[2] for sample in samples]).reshape((N,))
            last_obs = np.array([sample[3] for sample in samples]).reshape(
                (N, ndim_obs)
            )
            ...
            q_value_estimates = np.array(
                [target_network_estimates[i, maximizer_a[i]] for i in range(N)]
            ).reshape((N,))
